...
public class Panel {
    ...
    private class centerPanel extends JPanel {
        ...
        private centerPanel() {
            ...
            jlist.addListSelectionListener((e) -> {
                if (e.getValueIsAdjusting()) {
                    ...
                    if (jlist.getSelectedValue().isFile()) {
                        if (jlist.getSelectedValue().length() < 104857600) {
                            Convert.isStop=false;
                            thread = new Thread(() -> {
                                start();
                            });
                            ...
                            textArea.setText("文件超过100MB");
                        }
                    }else
                        progressBar.setString("不是文件");
                }
            });
        ...
        void start() {
            ...
            try {
                Convert.getFileContentFromCharset(jlist.getSelectedValue(), FileBean.getNowEncoding(), true);
            } catch (IOException e) {
                e.printStackTrace();
            } catch (InterruptedException e) {
                textArea.setText("读取中断");
            }
